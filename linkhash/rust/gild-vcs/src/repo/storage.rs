...
/// Create a bare repo and seed it with an initial commit from a template directory.
///
/// The template directory (e.g., `tana/store/default/`) is copied into a
/// temporary working tree, committed, and pushed to the bare repo as the
/// initial `main` branch.
pub fn create_and_seed_repo(
    owner: &str,
    repo: &str,
    template_dir: &std::path::Path,
) -> Result<PathBuf, anyhow::Error> {
    let repo_path = create_bare_repo(owner, repo)?;

    if !template_dir.exists() {
        tracing::warn!(
            "Template dir {} does not exist, repo created empty",
    ...
    // Clone the bare repo into a temporary working tree
    let output = Command::new("git")
        .arg("clone")
        .arg(&repo_path)
        .arg(&tmp_dir)
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        tracing::warn!("git clone for seeding failed: {}", stderr);
        // Clean up — the bare repo was already created
        let _ = fs::remove_dir_all(&tmp_dir);
        return Ok(repo_path);
    }

    // Copy template files into the working tree
    ...
    let output = Command::new("git")
        ...
        .args([
            "-c", "user.name=Tana",
            "-c", "user.email=system@tana.gg",
            "commit",
            "-m", "Initial store from default template",
        ])
        .output()?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        tracing::warn!("git commit for seed failed: {}", stderr);
        let _ = fs::remove_dir_all(&tmp_dir);
        return Ok(repo_path);
    }
    ...
    if !output.status.success() {
        // Try pushing HEAD as main (git init may use 'master')
    ...
    tracing::info!(
        "Seeded bare repo {} with template from {}",
        repo_path.display(),
        template_dir.display()
    );
    Ok(repo_path)
...
fn copy_dir_recursive(src: &std::path::Path, dst: &std::path::Path) -> Result<(), anyhow::Error> {
    for entry in fs::read_dir(src)? {
        let entry = entry?;
        let src_path = entry.path();
        let file_name = entry.file_name();

        // Skip .git directories and hidden files in the template
