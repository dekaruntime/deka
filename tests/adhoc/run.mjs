#!/usr/bin/env bun
// ADHOC scenarios for the one-command gate. These are product paths, not corpus fixtures.
...
const NAMES = [
  { name: "deka-init", title: "deka init writes a DekaScript app-router project ready to serve" },
...
async function main() {
  ...
  if (args.help) {
    console.log(`usage: bun tests/adhoc/run.mjs [--list] [--json] [--filter substr]`);
    process.exit(0);
  }
  if (args.list) {
