...
fn run_web_project_build(context: &Context) -> Result<(), String> {
    ...
    if api_dir.is_dir() {
        ...
        if !api_entries.is_empty() {
            write_cloudflare_worker(&dist_root, &api_entries)?;
        }
    }
...
fn write_cloudflare_worker(
    dist_root: &Path,
    api_entries: &[runtime_core::framework::FrameworkEntry],
) -> Result<(), String> {
    let mut routes = Vec::new();
    for api in api_entries {
        let methods = runtime_core::framework::exported_http_methods(Path::new(&api.file));
        ...
        routes.push(format!(
            "    [{}, {}],",
            serde_json::to_string(&api.route).unwrap_or_else(|_| "\"/api\"".into()),
            serde_json::to_string(&methods).unwrap_or_else(|_| "[]".into())
        ));
    }
    let source = format!(
        r#"// Generated by deka build. Worker is provisioned because api/ exists.
const ROUTES = [
{routes}
];
...
export default {{
  async fetch(request, env) {{
    ...
    const match = ROUTES.find((row) => row[0] === path);
    if (match) {{
      if (!match[1].includes(method)) {{
        return new Response("Method not allowed", {{ status: 405 }});
      }}
      if (env && env.DEKA_API) {{
        return env.DEKA_API.fetch(request);
      }}
      return new Response("API route " + method + " " + path, {{ status: 200 }});
    }}
