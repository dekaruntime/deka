...
use crate::compile_helper::{compile_or_report, compiler_version_from_context};
...
fn run(context: &Context) -> Result<(), String> {
    ...
    let report = compile_or_report(&source, input, parse_source_module_meta(&source), compiler)?;
