...
pub fn compile_graph_with_dsc(
    ...
    // dsc owns catalog validation, lowering, and helper emission. Compile the
    // original graph so package integrity and diagnostics use the real sources.
    let compile_root = project_root;
    let compile_entry = if entry.is_absolute() {
        entry.to_path_buf()
    } else {
