...
fn print_usage() {
    eprintln!(
        "Usage: wit-deka <module.wit> [--out path] [--world name] [--records struct|object] [--no-interface-prefix]"
    );
}
...
fn default_output_path(input: &Path) -> PathBuf {
    ...
    if let Some(base) = file_name.strip_suffix(".wit") {
        path.set_file_name(format!("{base}.d.ds"));
        return path;
    }
    path.set_file_name(format!("{file_name}.d.ds"));
    path
}
...
fn emit_stub(resolve: &Resolve, world: &World, args: &Args) -> Result<String> {
    ...
    out.push_str("// Generated by wit-deka. Do not edit by hand.\n");
    out.push_str("// This file is for DekaScript tooling only (type stubs).\n");
