...
mod tests {
    ...
    fn default_scaffold_declares_phase_aware_dev_permissions_and_denies_prod() {
        ...
        if let FsGrant::Paths(paths) = &permissions.dev.caps.write {
            // deka#1065: the compiler cache is one canonical root nested
            // under ds_modules (never a top-level .cache -- Sami does not
            // want the cache polluting the project root), so that is the
            // only write grant the scaffold needs to declare.
            assert!(paths.iter().any(|p| p == "ds_modules/.cache"));
            assert!(
                !paths.iter().any(|p| p == ".cache"),
                "scaffold must not grant write to a top-level .cache — the compiler cache lives under ds_modules/.cache instead: deka#1065"
            );
        }
