...
pub fn generate_app_router_entry_source(
    ...
    // let the dist-HTML rewrite and server-entry rewrite (build) swap in the
    // inlined JSON once the assets exist.
    let index_html = if !deferred.is_empty() && !index_html.contains("type=\"importmap\"") {
    ...
    generate_serve_entry(entry, &manifest, &index_html, &scripts, &defer_secret_js)
}
fn generate_serve_entry(
    ...
    let not_found_head = not_found_head_inner;
