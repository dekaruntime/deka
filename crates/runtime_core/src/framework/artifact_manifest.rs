...
impl ArtifactManifestV2 {
            ...
            while let Some(dir) = stack.pop() {
                ...
                for entry in entries {
                    ...
                    let file_type = entry.file_type().map_err(|err| {
                        format!("file_type error for {}: {err}", path.display())
                    })?;
                    if file_type.is_dir() {
                        stack.push(path);
    ...
    /// Read `dist/build-manifest.json` and run the cheap pre-parse checks in
    /// §3.4 order (sidecar, format, compat, payload_root, declared-module
    /// presence). The eager per-payload digest walk is [`Self::verify`]; lazy
    /// per-read verification lands with the artifact-only loader (deka#763).
