...
Capability registration confirms this — `crates/runtime_core/src/security.rs:110-114`:
...
- `crates/cli/src/cli/init.rs:143,155` — `deka init` scaffolding
  ...
  same function.
- `crates/php-rs/src/bin/common/mod.rs:37` — `run_script` entry
  point used by `deka run`, executes once at process startup before any user
  code runs.

The platform server (`crates/runtime/src/platform.rs`,
...
(`crates/runtime/src/js_pipeline.rs:40-49`) using the canonicalised
