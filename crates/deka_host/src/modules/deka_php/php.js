...
const { op_php_get_wasm, op_php_parse_phpx_types, op_php_read_file_sync, op_php_write_file_sync, op_php_mkdirs, op_php_set_privileged, op_php_sha256, op_php_random_bytes, op_php_digest, op_php_hmac, op_php_secure_compare, op_php_bcrypt_verify, op_php_db_call_proto, op_php_db_proto_encode, op_php_db_proto_decode, op_php_net_call_proto, op_php_net_proto_encode, op_php_net_proto_decode, op_php_fs_call_proto, op_php_fs_proto_encode, op_php_fs_proto_decode, op_php_cwd, op_php_file_exists, op_php_path_resolve, op_php_read_dir, op_php_parse_wit } = Deno.core.ops;
const console = {
    log: (...args)=>{
...
if (!globalThis.fs.writeFileSync) {
    globalThis.fs.writeFileSync = (path, data, encoding)=>{
        if (typeof op_php_write_file_sync === 'function') {
            const bytes = data instanceof Uint8Array
                ? data
                : typeof data === 'string'
                    ? new TextEncoder().encode(encoding && encoding !== 'utf8' && encoding !== 'utf-8' ? String(data) : data)
                    : new TextEncoder().encode(String(data));
            op_php_write_file_sync(path, bytes);
            return;
        }
        throw new Error('writeFileSync is not available in this runtime');
    };
}
if (!globalThis.fs.mkdirSync) {
    globalThis.fs.mkdirSync = (path, options)=>{
        if (typeof op_php_mkdirs === 'function') {
            op_php_mkdirs(path);
            return;
        }
        throw new Error('mkdirSync is not available in this runtime');
    };
}
if (globalThis.Deno) {
    if (!globalThis.fs.mkdirSync) {
    ...
    if (globalThis.Deno.writeFileSync && !globalThis.fs.writeFileSync) {
        globalThis.fs.writeFileSync = (path, data, encoding)=>{
...
function hashSource(source) {
    if (typeof op_php_sha256 === 'function') {
        return op_php_sha256(String(source));
    }
    throw new Error('sha256 op is not available in this runtime');
}
...
function resolveModuleCacheRoot(entryPath) {
    const modulesRoot = resolvePhpModulesRoot(entryPath);
    if (!modulesRoot) return '';
    return globalThis.path.resolve(modulesRoot, '.cache', 'phpx');
}
...
function ensureDirForFile(filePath) {
    if (!filePath) return;
    const dir = globalThis.path.dirname(filePath);
    ensureDir(dir);
}
function resolveLockPath(entryPath) {
    const projectRoot = resolveProjectRoot(entryPath);
    if (!projectRoot) return '';
    return globalThis.path.resolve(projectRoot, 'deka.lock');
}
...
function buildLazyModuleRegistry(modules, entryPath) {
    let out = '';
    out += "if (!isset($GLOBALS['__PHPX_LAZY'])) { $GLOBALS['__PHPX_LAZY'] = array(); }\n";
    out += "if (!function_exists('__phpx_register_lazy')) { function __phpx_register_lazy($moduleId, $path, $deps) { $GLOBALS['__PHPX_LAZY'][$moduleId] = array('path' => $path, 'deps' => $deps); } }\n";
    const modulesRoot = resolvePhpModulesRoot(entryPath);
    if (!modulesRoot) {
        throw new Error("Missing php_modules/. Run `deka init` to create a project.");
    }
    ...
    for (const module of modules.values()){
        ...
        if (!cacheEntry || cacheEntry.hash !== desiredHash || cacheEntry.compiler !== PHPX_COMPILER_ID || cacheEntry.cache !== cacheRel || cacheEntry.src !== srcRel || cacheEntry.lockIdentity !== lockIdentity) {
            ...
            lockDirty = true;
        } else if (!Array.isArray(cacheEntry.deps) || JSON.stringify(cacheEntry.deps) !== JSON.stringify(deps)) {
            cacheEntry.deps = deps;
            lockDirty = true;
        } else if (!Array.isArray(cacheEntry.exports) || JSON.stringify(cacheEntry.exports) !== JSON.stringify(module.exports || [])) {
            cacheEntry.exports = module.exports || [];
            lockDirty = true;
        }
        const depsList = deps.map((dep)=>`'${escapePhpString(dep)}'`).join(', ');
        const depsPhp = deps.length === 0 ? 'array()' : `array(${depsList})`;
        out += `__phpx_register_lazy('${escapePhpString(module.moduleId)}', '${escapePhpString(cachePath)}', ${depsPhp});\n`;
    }
    if (lockDirty) {
        writeDekaLock(lockInfo.lockPath, lock);
    }
    return out;
}
...
function buildPhpxBridgePrelude() {
    ...
    out += "  }\n";
    out += "} }\n";
    out += "if (!function_exists('__phpx_dynamic_allowed')) { function __phpx_dynamic_allowed() {\n";
    out += "  $enforce = getenv('DEKA_SECURITY_ENFORCE');\n";
    out += "  if (!is_string($enforce) || $enforce !== '1') { return true; }\n";
    out += "  $raw = getenv('DEKA_SECURITY_POLICY');\n";
    out += "  if (!is_string($raw) || $raw === '') { return false; }\n";
    out += "  $decoded = json_decode($raw, true);\n";
    out += "  if (!is_array($decoded)) { return false; }\n";
    out += "  $sec = __phpx_array_get($decoded, 'security');\n";
    out += "  if (!is_array($sec)) { return false; }\n";
    out += "  $allow = __phpx_array_get($sec, 'allow', array());\n";
    out += "  $deny = __phpx_array_get($sec, 'deny', array());\n";
    out += "  $allow_dynamic = is_array($allow) && !empty($allow['dynamic']);\n";
    out += "  $deny_dynamic = is_array($deny) && !empty($deny['dynamic']);\n";
    out += "  return $allow_dynamic && !$deny_dynamic;\n";
    out += "} }\n";
    ...
    out += "  if (!isset($GLOBALS['__PHPX_LOAD_STATUS'])) { $GLOBALS['__PHPX_LOAD_STATUS'] = array(); }\n";
    out += "  if (!isset($GLOBALS['__PHPX_LOAD_STACK'])) { $GLOBALS['__PHPX_LOAD_STACK'] = array(); }\n";
    out += "  if (__phpx_array_has_key($GLOBALS['__PHPX_LOAD_STATUS'], $moduleId) && $GLOBALS['__PHPX_LOAD_STATUS'][$moduleId] === 'loading') {\n";
    out += "    $path = implode(' -> ', array_merge($GLOBALS['__PHPX_LOAD_STACK'], array($moduleId)));\n";
    out += "    return __phpx_fail('Cyclic phpx import detected: ' . $path);\n";
    out += "  }\n";
    out += "  $GLOBALS['__PHPX_LOAD_STATUS'][$moduleId] = 'loading';\n";
    out += "  $GLOBALS['__PHPX_LOAD_STACK'][] = $moduleId;\n";
    ...
    out += "  if (isset($entry['path'])) { require $entry['path']; }\n";
    out += "  else if (isset($entry['code'])) { if (!__phpx_dynamic_allowed()) { return __phpx_fail('SECURITY_DYNAMIC_EXEC_DENIED: dynamic module eval is blocked'); } eval($entry['code']); }\n";
    out += "  array_pop($GLOBALS['__PHPX_LOAD_STACK']);\n";
    out += "  if (!isset($GLOBALS['__PHPX_MODULES'][$moduleId])) {\n";
    out += "    unset($GLOBALS['__PHPX_LOAD_STATUS'][$moduleId]);\n";
    out += "    return __phpx_fail('Failed to load phpx module: ' . $moduleId);\n";
    out += "  }\n";
    out += "  $GLOBALS['__PHPX_LOAD_STATUS'][$moduleId] = 'loaded';\n";
    out += "} }\n";
...
function buildModulePrelude(entryPath) {
    ...
    if (!enableModules) {
        ...
        if (moduleBuild.modules.size > 0) {
            prelude += buildLazyModuleRegistry(moduleBuild.modules, entryPath);
        }
...
function routeHostCall(kind, action, payload) {
    ...
    if (kind === 'crypto') {
        ...
        if (act === 'random_bytes') {
            ...
            let filled = false;
            if (!filled && typeof op_php_random_bytes === 'function') {
                const raw = op_php_random_bytes(Math.floor(n));
                if (raw && typeof raw.length === 'number') {
                    bytes.set(raw);
                    filled = true;
                }
            }
            if (!filled && g && g.crypto && typeof g.crypto.getRandomValues === 'function') {
            ...
            if (!filled && typeof require === 'function') {
                ...
                // no-op, we'll fail with a clear error below
                }
            }
            if (!filled) {
                return {
        ...
        if (act === 'bcrypt_verify') {
            const req = payload || {};
            if (typeof op_php_bcrypt_verify !== 'function') {
                return {
                    ok: false,
                    error: 'op_php_bcrypt_verify unavailable'
                };
            }
            const decoded = op_php_bcrypt_verify(String(req.password ?? ''), String(req.hash ?? ''));
            // Return entries to avoid host object-wrapper conversion on PHPX side.
            return Object.entries(decoded || {});
        }
        const toBytes = (v) => {
        ...
        if (act === 'digest') {
            ...
            if (typeof op_php_digest !== 'function') {
                return { ok: false, error: 'op_php_digest unavailable' };
            }
            const raw = op_php_digest(algorithm, data);
        ...
        if (act === 'hmac') {
            ...
            if (typeof op_php_hmac !== 'function') {
                return { ok: false, error: 'op_php_hmac unavailable' };
            }
            const raw = op_php_hmac(algorithm, key, data);
            const okVal = raw && raw.ok === true;
            if (okVal) {
                const arr = raw.data instanceof Uint8Array ? Array.from(raw.data) : (Array.isArray(raw.data) ? raw.data : Array.from(raw.data || []));
                return Object.entries({ ok: true, data: arr });
        ...
        if (act === 'secure_compare') {
            ...
            if (typeof op_php_secure_compare === 'function') {
                const raw = op_php_secure_compare(a, b);
                const okVal = raw && raw.ok === true;
                if (okVal) {
...
function phpHostCall(kindPtr, kindLen, actionPtr, actionLen, payloadPtr, payloadLen) {
    try {
        if (!phpMemory) {
            throw new Error('php wasm memory unavailable');
        }
        const normalizeToken = (value)=>String(value || '').replace(/\0/g, '').trim();
        const kind = normalizeToken(readString(kindPtr, kindLen));
        const action = normalizeToken(readString(actionPtr, actionLen));
        const payloadJson = readString(payloadPtr, payloadLen);
        let payload = null;
        if (payloadJson) {
            try {
        ...
        let result = null;
        try {
            result = routeHostCall(kind, action, payload);
        } catch (err) {
...
function phpWasmCall(modulePtr, moduleLen, exportPtr, exportLen, argsPtr, argsLen) {
    try {
        ...
        const normalizeToken = (value)=>String(value || '').replace(/\0/g, '').trim();
        const moduleId = normalizeToken(readString(modulePtr, moduleLen));
        const exportName = normalizeToken(readString(exportPtr, exportLen));
        ...
        if (isDbBridge || isNetBridge || isFsBridge || isCryptoBridge || isTimeBridge || isLegacyHostBridge) {
            ...
            let result = null;
            try {
                if (isDbBridge || moduleId.includes('db')) {
                ...
                } else if (isNetBridge || moduleId.includes('net')) {
                    ...
                    result = {
                        ok: false,
                        error: `unknown host bridge module '${moduleId}'`
                    };
                }
            } catch (err) {
                const message = err && err.message ? err.message : String(err);
...
function buildPrelude(request, filePath) {
    ...
    const envEntries = Object.entries(globalThis.process?.env || {}).map(([key, value])=>[
            String(key),
            ...
            try {
                ...
                if (result && result.ok) {
                    ...
                    if (Array.isArray(result.headers)) {
                        for (const headerLine of result.headers){
                    ...
                    let statusCode = Number(result.status || 0);
