...
CURRENT="$(scripts/runtime-version.sh)"
...
if [[ -n "$LATEST_TAG" ]]; then
  ...
  if [[ "$LATEST_VER" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] && ! semver_gt "$NEW" "$LATEST_VER"; then
    ...
    echo "v0.26.1 and v0.28.0 were tagged on trees that still said an older version;" >&2
...
if n != 1:
    if "[workspace.package]" in text:
        ...
        sys.exit(1)
    # Insert the table after the members list.
    new_text, n = re.subn(
        r"(?m)^(members = \[[^\]]*\]\n)",
        r"\1\n[workspace.package]\nversion = \"" + new + "\"\n",
        text,
    ...
    if n != 1:
        sys.stderr.write("failed to insert [workspace.package]\n")
        sys.exit(1)
