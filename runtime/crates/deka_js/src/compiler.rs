...
pub fn compile_phpx_source_to_js_with_warnings_detailed(
    ...
    meta.is_ds = meta.is_ds || path.extension().and_then(|ext| ext.to_str()) == Some("ds");
    let result = compile_deka(source, input, &arena);
    ...
    let program = result.ast.ok_or_else(|| {
        CompileError::Other("no AST available after validation".to_string())
    })?;
    let js = emit_js_from_ast(&program, source.as_bytes(), meta)
