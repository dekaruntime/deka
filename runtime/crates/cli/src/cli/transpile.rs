...
fn transpile_file(
    ...
    if mode == TranspileMode::Bundle {
        write_bundle(input, &output, treeshake)?;
    } else {
        write_module(input, &output, treeshake)?;
    }
...
fn transpile_directory(
    ...
    match mode {
        TranspileMode::Bundle => {
            ...
            write_bundle(&entry, output, treeshake)?;
            stdio::success(&format!(
                "transpiled {} -> {}",
                input.display(),
                output.display()
        ...
        TranspileMode::Preserve => {
            ...
            let outputs = sources
                ...
                .map(|source| {
                    source
                        .strip_prefix(input)
                        .map(|rel| output_root.join(rel).with_extension("js"))
                        .map_err(|_| "failed to preserve source tree".to_string())
                })
                .collect::<Result<Vec<_>, _>>()?;
            preflight_outputs(&sources, &outputs)?;
            for (source, output) in sources.iter().zip(outputs) {
                write_module(source, &output, treeshake)?;
            }
            stdio::success(&format!(
                "transpiled {} -> {}",
                input.display(),
...
fn write_module(input: &Path, output: &Path, treeshake: bool) -> Result<(), String> {
    let mut js = compile_source(input)?;
    js = rewrite_relative_ds_imports(js);
    if treeshake {
        js = optimize_emitted_module(&js, output)?;
    }
    write_generated(output, &js)
}

fn write_bundle(input: &Path, output: &Path, treeshake: bool) -> Result<(), String> {
...
fn write_generated(output: &Path, js: &str) -> Result<(), String> {
    ...
    fs::write(output, format!("{GENERATED_MARKER}{js}"))
...
fn preflight_outputs(sources: &[PathBuf], outputs: &[PathBuf]) -> Result<(), String> {
