...
impl<'a> JsSubsetEmitter<'a> {
    ...
    fn finish(self) -> String {
        ...
        out.push_str("globalThis.__deka_ord ??= (value) => { const str = String(value ?? ''); return str.length ? str.charCodeAt(0) : 0; };\n");
        out.push_str("globalThis.chr ??= globalThis.__deka_chr;\n");
        out.push_str("globalThis.ord ??= globalThis.__deka_ord;\n");
        out.push_str("globalThis.strlen ??= (value) => String(value ?? '').length;\n");
        out.push_str("globalThis.substr ??= (value, start, length = null) => { const src = String(value ?? ''); let s = Number(start) || 0; if (s < 0) s = Math.max(src.length + s, 0); if (length === null || length === undefined) return src.slice(s); let l = Number(length) || 0; if (l < 0) return src.slice(s, Math.max(src.length + l, s)); return src.slice(s, s + l); };\n");
        out.push_str("globalThis.ltrim ??= (value, chars = null) => { const src = String(value ?? ''); if (chars === null || chars === undefined || chars === '') return src.replace(/^\\s+/, ''); const set = new Set(String(chars)); let i = 0; while (i < src.length && set.has(src[i])) i += 1; return src.slice(i); };\n");
        out.push_str("globalThis.rtrim ??= (value, chars = null) => { const src = String(value ?? ''); if (chars === null || chars === undefined || chars === '') return src.replace(/\\s+$/, ''); const set = new Set(String(chars)); let i = src.length - 1; while (i >= 0 && set.has(src[i])) i -= 1; return src.slice(0, i + 1); };\n");
        out.push_str("globalThis.trim ??= (value, chars = null) => globalThis.ltrim(globalThis.rtrim(value, chars), chars);\n");
        out.push_str("globalThis.strpos ??= (haystack, needle, offset = 0) => { const h = String(haystack ?? ''); const n = String(needle ?? ''); const i = h.indexOf(n, Number(offset) || 0); return i >= 0 ? i : false; };\n");
        out.push_str("globalThis.strrpos ??= (haystack, needle, offset = null) => { const h = String(haystack ?? ''); const n = String(needle ?? ''); const i = offset === null || offset === undefined ? h.lastIndexOf(n) : h.lastIndexOf(n, Number(offset) || 0); return i >= 0 ? i : false; };\n");
        out.push_str("globalThis.str_starts_with ??= (haystack, needle) => String(haystack ?? '').startsWith(String(needle ?? ''));\n");
        out.push_str("globalThis.str_ends_with ??= (haystack, needle) => String(haystack ?? '').endsWith(String(needle ?? ''));\n");
        out.push_str("globalThis.str_contains ??= (haystack, needle) => String(haystack ?? '').includes(String(needle ?? ''));\n");
        out.push_str("globalThis.strtolower ??= (value) => String(value ?? '').toLowerCase();\n");
        out.push_str("globalThis.strtoupper ??= (value) => String(value ?? '').toUpperCase();\n");
        out.push_str("globalThis.array_key_exists ??= (key, value) => { if (value === null || value === undefined) return false; if (Array.isArray(value)) { const idx = Number(key); return Number.isInteger(idx) && idx >= 0 && idx < value.length; } if (typeof value === 'object') return Object.prototype.hasOwnProperty.call(value, key); return false; };\n");
        out.push_str("globalThis.in_array ??= (needle, haystack, strict = false) => { if (!Array.isArray(haystack)) return false; for (const item of haystack) { if (strict ? item === needle : item == needle) return true; } return false; };\n");
        out.push_str("globalThis.explode ??= (sep, value, limit = null) => { const s = String(value ?? ''); const parts = s.split(String(sep ?? '')); if (limit === null || limit === undefined || Number(limit) === 0) return parts; const n = Number(limit); if (!Number.isInteger(n)) return parts; if (n > 0) { if (parts.length <= n) return parts; const head = parts.slice(0, n - 1); head.push(parts.slice(n - 1).join(String(sep ?? ''))); return head; } return parts; };\n");
        out.push_str("globalThis.implode ??= (glue, pieces = null) => { if (pieces === null || pieces === undefined) { pieces = glue; glue = ''; } if (!Array.isArray(pieces)) return ''; return pieces.map((v) => String(v ?? '')).join(String(glue ?? '')); };\n");
        out.push_str("globalThis.count ??= (value) => { if (Array.isArray(value) || typeof value === 'string') return value.length; if (value && typeof value === 'object') return Object.keys(value).length; return 0; };\n");
        out.push_str("globalThis.time ??= () => Math.floor(Date.now() / 1000);\n");
