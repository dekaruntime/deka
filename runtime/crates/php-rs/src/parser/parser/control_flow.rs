...
impl<'src, 'ast> Parser<'src, 'ast> {
    ...
    fn validate_break_continue_level(&mut self, expr: ExprId<'ast>) {
        if let Expr::Integer { value, span } = expr {
            ...
            self.errors.push(crate::parser::ast::ParseError::new(
                expr.span(),
                "break/continue level must be a positive integer literal",
            ));
        }
    }
    ...
    fn validate_declare_item(&mut self, key: &Token, value: ExprId<'ast>) {
        if self.token_eq_ident(key, b"strict_types") {
            ...
            if self.seen_non_declare_stmt {
                self.errors.push(crate::parser::ast::ParseError::new(
                    key.span,
                    "strict_types declaration must be the first statement in the file",
                ));
            }
            ...
            if let Some(num) = self.int_literal_value(value) {
                if num != 0 && num != 1 {
                    self.errors.push(crate::parser::ast::ParseError::new(
                        value.span(),
                        "strict_types must be 0 or 1",
                    ));
                }
            } else {
                self.errors.push(crate::parser::ast::ParseError::new(
                    value.span(),
                    "strict_types must be an integer literal",
                ));
            }
            ...
            if let Some(num) = self.int_literal_value(value) {
                ...
                self.errors.push(crate::parser::ast::ParseError::new(
                    value.span(),
                    "ticks must be an integer literal",
                ));
            }
        } else if self.token_eq_ident(key, b"encoding") {
            match value {
                Expr::String { .. } => {}
                _ => self.errors.push(crate::parser::ast::ParseError::new(
                    value.span(),
                    "encoding must be a string literal",
                )),
            }
        }
    }
