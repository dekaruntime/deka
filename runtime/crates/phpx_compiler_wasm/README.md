...
`phpx_compiler_wasm` is the internal crate implementing the versioned Deka
...
- `deka_compiler_alloc(size)` and `deka_compiler_free(ptr, size)` own all input
  and result buffers.
- `deka_compiler_compile(source, filename, mode)` accepts UTF-8 byte ranges.
  `mode` is `auto` or `deka`; input filenames must end in `.ds`. Both modes
  use the native DekaScript parser mode. PHPX modes, filenames, and ABI aliases
  are intentionally unsupported.
- The result is a pointer to `{ ptr: u32, len: u32 }`, followed by UTF-8 JSON:
  `{"abi_version":1,"ok":...,"output":{"code":...},"diagnostics":[...],"metadata":...}`.
...
Run `runtime/scripts/build-deka-compiler-wasm.sh [output-dir]` from a clean
checkout. It refuses dirty source trees, uses the runtime's pinned Rust
toolchain, `cargo --locked`, a clean incremental setting, and embeds the exact
full Git commit through `DEKA_SOURCE_COMMIT`. It writes:
...
Run `runtime/scripts/test-deka-compiler-wasm.sh` to exercise native response
