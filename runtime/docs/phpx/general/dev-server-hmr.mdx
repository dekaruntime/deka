...
- `port <n> is already in use... deka serve refuses to share ports between processes`

that means another local server is already bound to that port. Stop it or run with `--port <n>`.
