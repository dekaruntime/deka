...
export const posts: Post[] = [
  ...
  },
  {
    ...
    "html": "<p>Application DekaScript cannot call <code>deka.*</code>. The generated serve entry can, and does: <code>unsafe { deka.ui.renderToStreamHtml(tree) }</code>. The 0.52 isolate bootstrap freezes <code>deka.ui</code> as signals-only.</p>\n<p>The subject app therefore installs <code>renderToString</code> through a module-scope <code>unsafe</code> block in the JSX runtime. That is a door: it runs once at module load, patches the host object, and gets out.</p>\n<pre class=\"code\"><code class=\"language-ds\"><span class=\"tok-kw\">const</span> <span class=\"tok-id\">_boot</span>: <span class=\"tok-id\">number</span> = <span class=\"tok-kw\">match</span> (<span class=\"tok-kw\">unsafe</span> {\n  <span class=\"tok-id\">globalThis</span>.<span class=\"tok-id\">deka</span>.<span class=\"tok-id\">ui</span> = <span class=\"tok-id\">Object</span>.<span class=\"tok-id\">assign</span>({}, <span class=\"tok-id\">globalThis</span>.<span class=\"tok-id\">deka</span>.<span class=\"tok-id\">ui</span>, {\n    <span class=\"tok-id\">renderToString</span>: <span class=\"tok-id\">renderToString</span>,\n    <span class=\"tok-id\">renderToStreamHtml</span>: <span class=\"tok-id\">renderToStreamHtml</span>\n  })\n  <span class=\"tok-kw\">return</span> <span class=\"tok-num\">1</span>\n}) {\n  <span class=\"tok-id\">Ok</span>(<span class=\"tok-id\">n</span>) =&gt; <span class=\"tok-id\">n</span>,\n  <span class=\"tok-id\">Err</span>(<span class=\"tok-id\">_</span>) =&gt; <span class=\"tok-num\">0</span>\n}</code></pre>\n<p>Posts, layouts, and islands stay in ordinary DSX. If the whole blog were an unsafe block, we would not be dogfooding the language.</p>"
  },
  {
    ...
    "html": "<p>Deka 0.52 emits <code>import { jsx } from \"&lt;jsxRuntime&gt;\"</code> into every file that contains JSX. Production artifacts then require that specifier to resolve to a relative <code>.js</code> module inside <code>dist/server</code>.</p>\n<p>The subject app therefore keeps JSX in <code>src/ui/<em>.dsx</code> — one directory depth that matches the generated serve entry — and keeps <code>app/</em>*/page.dsx</code> as thin wrappers that call those components.</p>\n<pre class=\"code\"><code class=\"language-ds\"><span class=\"tok-kw\">import</span> { <span class=\"tok-id\">HomePage</span> } <span class=\"tok-id\">from</span> <span class=\"tok-str\">\"../src/ui/HomePage.dsx\"</span>\n<span class=\"tok-kw\">export</span> <span class=\"tok-kw\">fn</span> <span class=\"tok-id\">Page</span>() <span class=\"tok-id\">ReactNode</span> {\n  <span class=\"tok-kw\">return</span> <span class=\"tok-id\">HomePage</span>()\n}</code></pre>\n<p>This is not how we want the authoring story to read in a year. It is how a closed artifact graph looks today, and hiding it would be a rigged screenshot.</p>"
  },
  {
  ...
  },
  {
    ...
    "html": "<p>This bench installs <code>deka 0.52.0</code> and <code>dsc 0.52.2</code> as release artifacts. It does not build the compiler from the commit that contains the bench. That would make the bench a moving target and the number a function of the PR.</p>\n<pre class=\"code\"><code class=\"language-bash\"><span class=\"tok-com\"># from https://deka.gg/install</span>\n<span class=\"tok-kw\">curl</span> -<span class=\"tok-id\">fsSL</span> <span class=\"tok-id\">https</span>:<span class=\"tok-com\">//deka.gg/install.sh | DEKA_VERSION=v0.52.0 DSC_VERSION=v0.52.2 sh</span></code></pre>\n<p>If you are iterating on the compiler, run the bench against the released pair anyway. A regression against yourself is a different chart.</p>"
  },
  {
